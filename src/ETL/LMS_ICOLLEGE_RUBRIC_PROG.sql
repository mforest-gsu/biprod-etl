...
SELECT
  ...
  (
    ...
    FROM
      MFOREST.D2L_RUBRIC_CRITERIA_LEVEL RubricCriteriaLevel
    WHERE
...
FROM
  MFOREST.LMS_RUBRIC_PROGRAM ProgramDept,
  MFOREST.D2L_RUBRIC_OBJECT RubricObject,
  MFOREST.D2L_RUBRIC_ASSESSMENT RubricAssessment,
  MFOREST.D2L_RUBRIC_ASSESSMENT_CRITERIA RubricAssessmentCriteria,
  (
    SELECT
      *
    FROM
      MFOREST.D2L_ORGANIZATIONAL_UNIT CourseOfferingOU
    WHERE
      CourseOfferingOU.IsDeleted = 0 AND
      CourseOfferingOU.Type = 'Course Offering'
  ) CourseOffering,
  MFOREST.D2L_ORGANIZATIONAL_UNIT Semester,
  MFOREST.D2L_USER UserAccount,
  MFOREST.D2L_RUBRIC_OBJECT_LEVELS RubricLevel,
  MFOREST.D2L_RUBRIC_OBJECT_CRITERIA RubricObjectCriteria,
  MFOREST.D2L_RUBRIC_OBJECT_LEVELS CriteriaLevel
WHERE
  -- RubricObject
...
USING
  (
    SELECT
      ...
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVCOLL' AND STVCODE_CODE = SGBSTDN_COLL_CODE_1) AS COLG_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVDEPT' AND STVCODE_CODE = SGBSTDN_DEPT_CODE) AS DEPT_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVDEGC' AND STVCODE_CODE = SGBSTDN_DEGC_CODE_1) AS DEGR_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVMAJR' AND STVCODE_CODE = SGBSTDN_MAJR_CODE_1) AS MAJR_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVMAJR' AND STVCODE_CODE = SGBSTDN_MAJR_CODE_CONC_1) AS CONCENT_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'USGRGNT' AND STVCODE_CODE = STU_LEVEL_REGENTS) AS LEVEL_REGENTS_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVLEVL' AND STVCODE_CODE = SGBSTDN_LEVL_CODE) AS STU_LEVEL_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVSTYP' AND STVCODE_CODE = SGBSTDN_STYP_CODE) AS STU_TYPE_DESC,
      (SELECT STVCODE_DESC FROM MFOREST.BNR_STVCODE WHERE STVCODE_TABLE = 'STVCAMP' AND STVCODE_CODE = SGBSTDN_STYP_CODE) AS PRIME_CAMP_DESC,
